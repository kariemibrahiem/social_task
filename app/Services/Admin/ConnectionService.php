...
{
    protected string $folder = 'content/connection';
    protected string $route = 'connections';
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($obj)
                ->addColumn('action', function ($obj) {
                    ...
                    if ($user && $user->can("connections_edit")) {
                        $buttons .= '
                    ...
                    if ($user && $user->can("connections_delete")) {
                        $buttons .= '
                                    <button type="button" 
                ...
                ->editColumn('sender', function ($obj) {
                    return $obj->sender?->name ?? "-";
                })
                ->editColumn('receiver', function ($obj) {
                    return $obj->receiver?->name ?? "-";
                })
                ->editColumn('status', function ($obj) {
                    return $obj->status == 'pending' ? '<span class="badge bg-warning">' . trns('pending') . '</span>' : ($obj->status == 'accepted' ? '<span class="badge bg-success">' . trns('accepted') . '</span>' : '<span class="badge bg-danger">' . trns('rejected') . '</span>');
                })
                ->addIndexColumn()
                ->escapeColumns([])
                ->make(true);
    ...
    {
        if (isset($data['image'])) {
            $data['image'] = $this->handleFile($data['image'], 'Connection');
        }
