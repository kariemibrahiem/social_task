...
if (!function_exists('getSetting')) {
    function getSetting($key, $guard = 'admin')
    {
        $model = GeneralSetting::query();

        if ($guard == 'vendor') {
            return $model->where('vendor_id', auth()->guard('vendor')->user()->id)
                ->where('key', $key)
                ->value('value');
        }

        return $model->where('guard_name', $guard)
            ->where('key', $key)
            ->value('value');
    }
}
