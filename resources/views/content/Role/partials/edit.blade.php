...
@section('title', trns('Edit Role'))

@section('content')
<h4 class="py-3 mb-4">
  <span class="text-muted fw-light">
    <a href="{{ route($route.'.index') }}">{{ trns('Roles') }}</a> /
  </span> {{ trns('Edit Role') }}
</h4>
      ...
      <div class="row g-3 mb-3">
        ...
        <div class="col-md-6">
          ...
          <input type="text" class="form-control @error('role') is-invalid @enderror" 
                 id="role" name="role" value="{{ old('role', $obj->name) }}" required>
          @error('role')
        ...
        <div class="col-md-12">
          ...
          @php
            ...
            $oldPermissions = old('permission_id', $obj->permissions->pluck('id')->toArray());
          @endphp
        ...
        <div class="col-md-6">
          ...
          <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
            <option value="1" {{ old('status', $obj->status) == '1' ? 'selected' : '' }}>{{ trns('Active') }}</option>
            <option value="0" {{ old('status', $obj->status) == '0' ? 'selected' : '' }}>{{ trns('Inactive') }}</option>
          </select>
          @error('status')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
